...
Last updated: 2026-08-27 (recall accuracy Phase 2 docs: one-hop typed relation expansion, prompt-time expansion caps, no weak expansion; recall accuracy Phase 1: key_conclusion/path-aware ranking, JSON explanations, low_measurability health; OpenCode memory injection observability, Edit Alignment semantics, project binding-norm subsystem)
...
"Advanced / lifecycle" means specialized or less frequent, never deprecated — a
Skill you reach for at a specific moment (installing, upgrading, compressing a
phase, synthesizing a topic) rather than in every session. Every Skill in the
second tier is fully supported and directly invocable; the tier is about
frequency, never about status.
...
| Codex | Partial runtime plus skills | User skills under `~/.agents/skills` plus managed `.codex/hooks/session_start.py`, `.codex/hooks/user_prompt.py`, `.codex/hooks/session_end.py`, `.codex/hooks/stop.py`, and `.codex/hooks/post_compact.py` registered under supported Codex hook events | Not skills-only anymore. Codex supports bounded startup project context, prompt-time project recall, User Habit Memory reminders, record-intent capture, bounded recall/memory observability, SessionEnd edit-alignment outcomes, loop-safe Stop record nudges, and compact re-seed markers through supported Codex seams. |
...
| Prompt-time recall observability log | Full where Claude prompt hook logs recall injection/abstention | Full on OpenCode seam: `.sybermem/.recall-debug.jsonl` appends bounded inject/abstain metadata without prompt text | Full on Codex hook seam: `UserPromptSubmit` appends bounded `.sybermem/.recall-debug.jsonl` inject/abstain metadata without prompt text | Unsupported | Unsupported | Unsupported |
| Memory injection observability log | Unsupported | Full on OpenCode seam: bounded `.sybermem/.memory-usage.jsonl` per-turn and `session_outcome` rows for memory that actually reached the model-visible prompt | Partial runtime: `UserPromptSubmit`/`SessionStart` append bounded `.sybermem/.memory-usage.jsonl` per-turn lane rows; `SessionEnd` can append best-effort `session_outcome` rows | Unsupported | Unsupported | Unsupported |
...
| In-session injection visibility | N/A (Claude injects into visible `UserPromptSubmit` context) | Full on OpenCode seam: throttled SEPARATE toasts at `experimental.chat.system.transform` time — `⭐` recall, `🧠` applied habit, `📏` applied project norm — plus a scope-aware `💡` toast when a prompt looks like a reusable preference/norm but no habit matched (`habit_preference_candidate`) | Partial runtime: installed hooks use Codex `statusMessage`; injected `additionalContext` starts with a count/id marker and asks the model to restate the per-turn count; dynamic `systemMessage` count is best-effort where Codex surfaces warnings/events. No OpenCode-style TUI toast or default Windows desktop toast is claimed | Unsupported | Unsupported | Unsupported |
...
| Installer coverage | Full: Claude skills, CLI/Core, global launchers | Full: OpenCode skills and plugin | Full: Codex skills, SessionStart/UserPromptSubmit/SessionEnd/Stop/PostCompact hooks, shared observability helper, hooks.json merge | Version/manifest only | Version/manifest only | Version/manifest only |
...
| Derived Project Index | Full | `sybermem project index build`, `sybermem project index check` | `.sybermem/INDEX.md` is a local, Git-ignored artifact generated from canonical records and mechanically checked; missing INDEX is a rebuild hint, not an uninitialized-project state. |
...
| Project Refresh | Full | `sybermem project refresh --format json`, `/sybermem-update` | Deterministic project-local managed-file propagation for `.sybermem/`, hooks, templates, `.claude/settings.json`, and `project.yaml`; refresh also untracks a tracked `.sybermem/INDEX.md` with `git rm --cached` while preserving the work file, plus removal of any legacy SyberMem protocol block from `CLAUDE.md` / `AGENTS.md`; `/sybermem-update` falls back to agent orchestration only when CLI refresh is unavailable or invalid. |
...
| Search | Full | `/sybermem-search`, `sybermem search` | Supports project/workspace search, record-id/topic/keyword/relation matching, `key_conclusion` first-class scoring, capped `related_files` path/module boosting, one-hop typed relation expansion with `relation-expanded` / `expanded_from` / `expansion_relation` provenance on machine-readable surfaces, successor guidance, conflict notes, and graceful degradation when local INDEX is missing (with rebuild guidance). |
| High-signal Recall | Full | Claude `UserPromptSubmit`, OpenCode `chat.message`/transform, Codex `UserPromptSubmit`, `sybermem context recall` | Automatic only on Claude/OpenCode/Codex supported prompt seams. Uses a stricter gate than explicit search: high-signal seeds may add at most one non-evidence one-hop relation expansion per seed, capped at two total expansions per prompt packet, while weak keyword-only, topic-only, and semantic-only matches never expand or inject. `context recall --format json` exposes matched fields, score breakdowns, and expansion provenance, while Markdown stays compact for prompt injection. |
| Workspace/Hub | Full | `sybermem index build`, workspace `sybermem search`, `sybermem portfolio` | SQLite FTS5 workspace index with project/type/status filters and stale-index detection; workspace and portfolio remain record/project.yaml driven when a local derived INDEX is missing. |
...
| Record Authoring | Skill-orchestrated | `/sybermem-record`; CLI helper `sybermem record id --type ...` | CLI mints IDs and validates indexes; full record writing and supported existing-record relation correction remain one skill workflow. |
...
| Prompt-time project recall | Full | `chat.message` -> `sybermem context recall` -> `experimental.chat.system.transform` | Same-turn system prompt injection. Only high-signal recall qualifies; once a high-signal seed qualifies, the shared gate may append at most one non-evidence one-hop relation expansion per seed and at most two total expansions per packet. Weak keyword-only, topic-only, and semantic-only matches stay out. |
...
| Session-start project context | Partial runtime | `.codex/hooks/session_start.py` registered under `SessionStart` | Emits shared `sybermem context session --format markdown` only when available, prefixed with `## SyberMem Codex Startup`; fail-open, bounded, and recorded in `.sybermem/.memory-usage.jsonl` as startup lane metadata. |
| Habit prompt reminder | Partial runtime | `.codex/hooks/user_prompt.py` registered under `UserPromptSubmit` | Composed with recall in one `hookSpecificOutput.additionalContext` packet, prefixed with `## SyberMem Codex Context` when anything is injected; only bounded `## User Habit Reminder` output qualifies. Injection counts are recorded in `.memory-usage.jsonl`. |
...
| Project memory prompt recall | Partial runtime | `UserPromptSubmit` -> `sybermem context recall --query <prompt>` | Uses shared high-signal recall gate and `⭐`/`💡` markers inside the recall section; qualifying packets start with a SyberMem Codex summary marker that includes lane counts and injected IDs. Inject/abstain metadata is recorded in `.recall-debug.jsonl`; abstentions emit no context. |
...
| Session lifecycle hooks | Partial runtime | Managed `SessionStart`, `UserPromptSubmit`, `SessionEnd`, `Stop`, and `PostCompact`; researched `PreCompact` is available in Codex | SyberMem uses only bounded command hooks; `SessionEnd` records best-effort edit-alignment outcomes, while hidden auto-resume and background workers are still unsupported. |
...
managed Codex hooks add model-visible SyberMem markers inside that same
`additionalContext` payload; prompt-time markers include lane counts, injected
IDs, and a restate instruction for Desktop perceptibility. Installers also use
Codex `statusMessage` for a hard visible status line, while dynamic
`systemMessage` counts remain best-effort because some Desktop builds swallow
hook warnings. SyberMem does not claim a Codex TUI toast API or install default
Windows desktop notifications. SyberMem uses `Stop` only for bounded record
nudges with `stop_hook_active` loop prevention and `SessionEnd` only for
best-effort observability outcomes.
