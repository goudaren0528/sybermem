...
| Project Memory Stats | Full | `sybermem project memory-stats`, `sybermem project memory-stats --format json`, `/sybermem-summary` | Deterministic 7d/30d memory and recall observability. Text mode prints terminal tables; JSON mode feeds `/sybermem-summary`. Recall metrics are backed only by `.sybermem/.recall-debug.jsonl`; no-log means unavailable, not zero recall activity. |
