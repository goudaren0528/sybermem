...
Last updated: 2026-08-17 (instruction-file injection removed; version/update notification; semantic norm nomination + opt-in reply marker)
...
| Semantic norm nomination | Skill-orchestrated | `/sybermem-habit`, `/sybermem-record`, `/sybermem-digest` | The Agent (not keywords) judges whether a statement is a reusable preference or a standing norm — including phrasing with no trigger word and constraints repeated ≥2× in a session/section. Personal/cross-project preference → user habit; project-specific convention → `decision`/`requirement` record. Batched, one-step-confirm, confirmation-first (L1), never mid-flow, dropped on decline. No new storage surface. |
...
| Injection visibility toasts | Full | `chat.message` + `experimental.chat.system.transform` | Throttled (~30s/type) and fail-open toasts, one per injection kind so habits are as perceptible as recall: a `⭐` recall toast, a SEPARATE `🧠` applied-habit toast, and a `💡` reusable-preference/norm toast when a candidate is captured. Never block or spam the prompt flow. |
