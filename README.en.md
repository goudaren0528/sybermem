...
- recall relevance feedback: at `session.idle`, OpenCode matches injected records against edited files (via `related_files`) into bounded `.sybermem/.recall-outcomes.jsonl` / `.memory-usage.jsonl` rows, yielding precision-based `low_relevance` and anchor-coverage-based `low_measurability` verdicts distinct from frequency. See the [Feature Map](docs/feature_map.md)
