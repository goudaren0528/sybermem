...
- **OpenCode**: re-run the installer (or `python scripts/update.py` in a checkout), then verify in stages: (1) installer result and deployed-file SHA-256 against build artifacts; (2) restart/reload the host and inspect its plugin loader; (3) send a real memory-matching prompt and check recall and visible feedback. Files alone cannot prove a toast or model use. V1 has a separate single-file `sybermem-v1.ts` entry; V2 needs the complete `package.json`/`server.js`/`tui.js` directory package. Do not dual-load old and new entries. See [OpenCode installation](.opencode/INSTALL.md).
