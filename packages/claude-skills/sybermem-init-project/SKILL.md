...
- If `.sybermem/project.yaml` already exists after resolution, treat the project as already initialized. A missing `.sybermem/INDEX.md` is only a missing local derived artifact, not an uninitialized project.
...
- If the project is a git repository (`.git` exists), add a marker-bounded SyberMem block to `.gitignore` that ignores machine-local runtime and scripts (`.sybermem/hooks/`, `.sybermem/` runtime dotfiles, `.claude/settings.json`) and ignores `.sybermem/INDEX.md` as a machine-local derived artifact regenerated by `sybermem project index build`, while keeping shareable memory (records, digests, analysis, templates, `project.yaml`) committable. Preserve any existing `.gitignore` content; skip entirely for non-git projects.
- Never copy project-files `.claude/settings.json` directly to an active project. Its `sybermem-template` entries are non-executable seeds; render them through `sybermem project refresh` only after confirming Claude Code supports exec hooks and the actual Python executable and global `launch_hook.py` exist. On failure, report ERROR/upgrade or repair and do not claim healthy installation. Preserve custom/third-party hooks.
...
- All four managed hooks (UserPromptSubmit, SessionStart, both Stop hooks) must use the verified absolute real Python executable as exec `command`, with absolute global `launch_hook.py`, hook ID, `--timeout-seconds` and safe child budget in `args`; do not publish relative executable settings. Use `sybermem project refresh` for surgical migration.
...
- Run `python .sybermem/hooks/check_project_health.py --read-only` for a no-write diagnostic; inspect `.claude/settings.json` `errors` and `actions_needed`. Never run real memory hooks as a health test. Unknown/old Claude Code, missing Python, launcher, target, relative managed hooks or unsafe timeout require ERROR and repair before declaring healthy.
...
- SyberMem ignore block added to `.gitignore` for git projects (machine-local runtime/scripts and derived `.sybermem/INDEX.md` ignored; records, digests, analysis, templates, and `project.yaml` kept committable; skipped for non-git projects)
