...
`status`/`refresh`/`memory-stats`, `resume`, `search`, manual `context` briefs, `portfolio`, `digest status`, workspace `index build`, project `index build`/`check`/`phase`/`coverage-hash`, user-habit `add`/`list`/`search`/`pause`/`delete`/`remind`/`inject`, and Team `init`/`summary`/`publish status`.
