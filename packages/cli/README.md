...
`sybermem index build` builds the workspace SQLite search index. `sybermem project index build` and `sybermem project index check` manage the derived `.sybermem/INDEX.md`. Record creation remains `/sybermem-record` skill orchestration, not a CLI command.
