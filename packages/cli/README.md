...
`chat.message` + `experimental.chat.system.transform`, writes `.memory-usage.jsonl`
only after model-visible injection succeeds, and emits one bounded post-injection
summary with total items and total chars for recall/habit/norm; startup context
keeps its separate one-shot notice. The
managed Codex `UserPromptSubmit` hook uses the same CLI route when Codex hook
support is installed. You can also invoke it manually to preview what would be
