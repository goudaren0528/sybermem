 ...
 * Regenerate with: bun scripts/build-opencode-plugin.mjs --package
 * Do not edit this file by hand; V2 server package entrypoint.
 */
// @bun
...
import { relative as relative2, resolve as resolve2, win32 } from "path";
...
async function setupSyberMemV2(ctx, shell = $) {
  ...
  let stopped = false;
  const controller = new AbortController;
  const aborted = new Promise((resolve3) => controller.signal.addEventListener("abort", () => resolve3(), { once: true }));
  const registrations = [];
  ...
  const client = { diagnostic, tui: { showToast: async ({ body }) => diagnostic(body.message) } };
  ...
  const sessionArgs = (sessionID) => ({ ...args, client: { diagnostic: (message) => {
    diagnostic(message);
    notice(sessionID, "advisory", message);
  }, tui: { showToast: async ({ body }) => {
    diagnostic(body.message);
    notice(sessionID, "advisory", body.message);
  } } } });
  async function belongs(sessionID) {
    ...
    try {
      ...
      if (stopped)
        return false;
      const canonical = (path) => process.platform === "win32" ? win32.normalize(win32.resolve(path)).toLowerCase() : resolve2(path);
  ...
  async function state(sessionID) {
    ...
    if (!value) {
      ...
      if (!pending) {
        pending = (async () => ({ startup: await buildStartupContext(shell, root) ?? "", packets: [], recalled: new Map, accounted: new Set }))();
        pendingStates.set(sessionID, pending);
      }
      try {
        value = await pending;
        if (!stopped && pendingStates.get(sessionID) === pending)
          sessions.set(sessionID, value);
      } finally {
        if (pendingStates.get(sessionID) === pending)
          pendingStates.delete(sessionID);
      }
    }
    return value;
  }
  const active = (sessionID, value) => !stopped && sessions.get(sessionID) === value;
  async function consume(sessionID, work) {
    ...
    try {
      ...
      release();
      if (contextQueues.get(sessionID) === done)
  ...
  const cleanup = async () => {
    stopped = true;
    controller.abort();
    await Promise.allSettled(registrations.splice(0).map(async (item) => {
      await item.dispose();
    }));
  ...
  try {
    ...
    registrations.push(await ctx.session.hook("prompt", async (event) => {
      if (!await belongs(event.sessionID))
        return;
      if (!stopped)
        admitted.set(event.sessionID, { id: event.messageID, text: event.prompt.text });
    }));
    registrations.push(await ctx.session.hook("context", async (event) => {
      ...
      await consume(event.sessionID, async () => {
        ...
        if (!active(event.sessionID, current) || injected.has(event))
          return;
        ...
        if (prompt?.id === admitted.get(event.sessionID)?.id)
          admitted.delete(event.sessionID);
        if (prompt && !current.recalled.has(prompt.id)) {
          const text2 = prompt.text || prompt.parts?.filter((part) => part.type === "text" && typeof part.text === "string").map((part) => part.text).join(`
`) || "";
          await captureRecordIntentWithCli(shell, root, text2);
          if (!active(event.sessionID, current))
            return;
          const habitIntent = await captureHabitIntentWithCli(shell, root, text2);
          if (!active(event.sessionID, current))
            return;
          const packets2 = await collectPromptPackets(shell, root, text2);
          if (!active(event.sessionID, current))
            return;
          current.packets = packets2;
          current.messageID = prompt.id;
          current.recalled.set(prompt.id, packets2);
          if (habitIntent.captured || classifyPackets(packets2).habitCandidate) {
            const message = habitCandidateToast(habitIntent);
            diagnostic(message);
            notice(event.sessionID, "advisory", message, prompt.id);
          }
          appendRecallDebug(root, packets2);
        }
        if (!active(event.sessionID, current))
          return;
        ...
        if (await injectPendingHabitReminder(shell, root, event.sessionID, output)) {
          ...
          if (active(event.sessionID, current)) {
            diagnostic(message);
            notice(event.sessionID, "advisory", message, messageID);
          }
        }
        if (!active(event.sessionID, current))
          return;
        injected.add(event);
        event.system.push(...output.system.map((text2) => ({ type: "text", text: text2 })));
    ...
    registrations.push(await ctx.session.hook("compaction", async (event) => {
      if (!await belongs(event.sessionID) || compacted.has(event))
        return;
      let text2 = null;
      try {
        text2 = await buildCompactionContext(shell, root);
      } catch {}
      if (text2 && !stopped && !compacted.has(event)) {
        compacted.add(event);
        if (!event.system.some((part) => part.type === "text" && part.text === text2))
          event.system.push({ type: "text", text: text2 });
      }
    }));
    registrations.push(await ctx.tool.hook("execute.after", async (event) => {
      if (event.status !== "completed" || !await belongs(event.sessionID))
        return;
      const current = await state(event.sessionID);
      if (!active(event.sessionID, current))
        return;
      ...
      if (["edit", "write"].includes(event.tool) && typeof input.filePath === "string") {
        recordEditedFile(event.sessionID, relative2(root, resolve2(ctx.location.directory, input.filePath)));
      }
      if (event.tool === "todowrite")
        recordTodoUpdate(event.sessionID, input);
  ...
  const events = (async () => {
    for await (const event of ctx.event.subscribe({ signal: controller.signal })) {
      ...
      try {
        await handleSessionIdle(scoped, root, id);
      } catch {}
      if (stopped)
        break;
    ...
    if (!stopped)
      console.warn("SyberMem V2 event subscription ended unexpectedly");
  });
  return async () => {
    await cleanup();
    await Promise.race([events, aborted]);
  };
}
var v2_default = { id: "sybermem", setup: setupSyberMemV2 };
export {
