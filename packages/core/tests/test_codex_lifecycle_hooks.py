...
def write_session_start_launcher(home: Path, *, context: str, digest_status: str = "", digest_latest: str = "") -> Path:
    assert_fake_launcher_home(home)
    launcher = home / ".claude" / "sybermem" / "cli" / ("sybermem.cmd" if os.name == "nt" else "sybermem")
    launcher.parent.mkdir(parents=True, exist_ok=True)
    script = home / "session-start-launcher.py"
    script.write_text(
        "from __future__ import annotations\n"
        "import sys\n"
        f"context = {context!r}\n"
        f"digest_status = {digest_status!r}\n"
        f"digest_latest = {digest_latest!r}\n"
        "args = sys.argv[1:]\n"
        "if 'context' in args and 'session' in args:\n"
        "    sys.stdout.write(context)\n"
        "elif 'digest' in args and 'status' in args:\n"
        "    sys.stdout.write(digest_status)\n"
        "elif 'digest' in args and 'latest' in args:\n"
        "    sys.stdout.write(digest_latest)\n"
        "raise SystemExit(0)\n",
        encoding="utf-8",
    )
    if os.name == "nt":
...
def test_codex_session_start_injects_current_latest_digest_conclusions(tmp_path: Path) -> None:
    project = create_sybermem_project(tmp_path)
    session_context = "## SyberMem Manual Session Context\n\n- Startup context."
    digest_status = json.dumps(
        {
            "total": 1,
            "current": 1,
            "stale": 0,
            "unknown": 0,
            "backlog": {"uncovered": 0, "has_digest": True, "days_since_latest_digest": 1},
            "digests": [{"record_id": "digest-002", "title": "Recent Phase", "verdict": "current", "reason": "ok", "drifted_sources": []}],
        }
    )
    digest_latest = json.dumps(
        {
            "record_id": "digest-002",
            "title": "Recent Phase",
            "date": "2026-08-10",
            "conclusions": ["- first", "- second", "- third", "- fourth", "- fifth", "- sixth"],
        }
    )
    write_session_start_launcher(tmp_path, context=session_context, digest_status=digest_status, digest_latest=digest_latest)
    ...
    assert "### Latest Digest: Recent Phase" in additional_context
    assert "- first" in additional_context
    assert "- fifth" in additional_context
    assert "- sixth" not in additional_context


def test_codex_session_start_omits_latest_digest_when_stale(tmp_path: Path) -> None:
    project = create_sybermem_project(tmp_path)
    session_context = "## SyberMem Manual Session Context\n\n- Startup context."
    digest_status = json.dumps(
        {
            ...
            "unknown": 0,
            "backlog": {"uncovered": 0, "has_digest": True, "days_since_latest_digest": 1},
            "digests": [{"record_id": "digest-002", "title": "Recent Phase", "verdict": "stale", "reason": "drift", "drifted_sources": []}],
        }
    )
    digest_latest = json.dumps(
        {
    ...
    output = json.loads(result.stdout)
    additional_context = output["hookSpecificOutput"]["additionalContext"]
    assert "### Latest Digest:" not in additional_context


def test_codex_session_start_omits_latest_digest_on_cli_failure(tmp_path: Path) -> None:
    project = create_sybermem_project(tmp_path)
    session_context = "## SyberMem Manual Session Context\n\n- Startup context."
    write_session_start_launcher(tmp_path, context=session_context, digest_status="not-json", digest_latest="not-json")
    payload = json.dumps({"hookEventName": "SessionStart", "source": "startup"})
