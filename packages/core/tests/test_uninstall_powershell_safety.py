...
def test_uninstall_ps1_preserves_unknown_files_and_cleans_all_skill_roots(tmp_path: Path) -> None:
    sentinel = _seed_home(tmp_path)
    result = subprocess.run(
        [POWERSHELL or "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(ROOT / "scripts" / "uninstall.ps1")],
        cwd=ROOT,
