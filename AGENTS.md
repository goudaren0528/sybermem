...
5. **Upgrade nudges**: In `auto` or `remind` mode, the stop hook may emit a non-blocking suggestion that a change looks important enough for `/sybermem-record`, or that a recent cluster of work may be ready for `/sybermem-digest`. These nudges are hints only and do not block exit.
