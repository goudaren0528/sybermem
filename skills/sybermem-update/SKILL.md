...
Current behavior note: OpenCode actual-injection observability requires refreshed global CLI/Core and a refreshed global OpenCode plugin (re-run the global install/update to refresh `~/.config/opencode/plugins/sybermem.ts`). The plugin writes `.sybermem/.memory-usage.jsonl` only after model-visible injection succeeds and emits one `prompt-memory-injected` summary toast for recall/habit/norm/startup lane counts. The `session.idle` recall-health advisory now surfaces both `low_signal` (recall rarely fires) and `low_relevance` (recall fires but injected records rarely match edited files). These features do not create, refresh, or migrate any project-local managed files through `sybermem project refresh --format json`; they read existing `.sybermem/` records and optional runtime logs, creating `.memory-usage.jsonl` only during OpenCode runtime use.
