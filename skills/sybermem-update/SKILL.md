...
Current behavior note: `sybermem project memory-stats` and the `/sybermem-summary` Memory Health / Recall Stats panel require refreshed global CLI/Core and skill instructions only. They do not create, refresh, or migrate any project-local managed files through `sybermem project refresh --format json`; the command reads existing `.sybermem/` records, optional `.sybermem/.recall-debug.jsonl` recall-frequency metadata, and optional `.sybermem/.recall-outcomes.jsonl` recall-precision metadata (used for the `low_relevance` verdict).

Current behavior note: the OpenCode first-turn startup-context injection and the `session.idle` recall-health advisory require a refreshed global OpenCode plugin only (re-run the global install/update to refresh `~/.config/opencode/plugins/sybermem.ts`). The advisory now surfaces both `low_signal` (recall rarely fires) and `low_relevance` (recall fires but injected records rarely match edited files). They do not create, refresh, or migrate any project-local managed files through `sybermem project refresh --format json`; they read existing `.sybermem/` records and the optional `.sybermem/.recall-debug.jsonl` / `.sybermem/.recall-outcomes.jsonl` metadata (both runtime logs, never scaffolded).
