...
- Boundary note: `sybermem-install` is a fully managed skill — it is part of the installer's `SKILLS` manifest and `managed-install.json`, so re-running install/update lands and refreshes it alongside every other SyberMem skill, and managed uninstall removes it. On a brand-new machine the very first copy still arrives via external distribution (the skill must exist before it can be triggered); after that first install, it is maintained by the normal install/update/uninstall paths like any other skill.
