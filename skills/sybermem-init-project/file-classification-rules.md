...
| `.gitignore` (git projects only) | Insert or refresh the bounded `# >>> SyberMem >>>` / `# <<< SyberMem <<<` block that ignores machine-local runtime/scripts (`.sybermem/hooks/`, `.sybermem/` runtime dotfiles, `.claude/settings.json`) and the machine-local derived `.sybermem/INDEX.md`. If the block exists, replace only its contents. All content outside the markers is preserved verbatim. Skip entirely for non-git projects. | Ignoring shareable memory (records/digests/analysis/templates/`project.yaml`); creating `.gitignore` in a non-git project; overwriting unrelated ignore rules. |
...
| `.sybermem/INDEX.md` | Treat as a machine-local, git-ignored derived artifact. Generate or regenerate it with `sybermem project index build`; whole-file deterministic regeneration is correct. Never hand-edit it. | Preserving or adding durable content directly in `INDEX.md`; hand-editing the generated file. |
